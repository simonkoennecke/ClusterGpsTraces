...
public final class Geometry
{
  ...
  public static double length (double x0, double y0, double x1, double y1)
  {
    ...
    */
	  //Formel: dist = 6378.388 * acos(sin(lat1) * sin(lat2) + cos(lat1) * cos(lat2) * cos(lon2 - lon1)) [KILOMETER]
	  //Formel: dist = 6378137 * acos(sin(x0) * sin(x1) + cos(x0) * cos(x1) * cos(y1 - y0)) [METER]
	  return 6378137 * Math.acos(
				Math.sin(Math.toRadians(x0)) * Math.sin(Math.toRadians(x1)) + 
				Math.cos(Math.toRadians(x0)) * Math.cos(Math.toRadians(x1)) * 
				Math.cos(Math.toRadians(y1) -  Math.toRadians(y0))
			);
	  //return PtOpSphere.distance(new Point(x0,y0), new Point(x1,y1));
  }
  ...
  public static double[] createVector (double[] p0, double[] p1)
  {
	...
	if(p0.length == 2){
		double v[] = {p1[0] - p0[0], p1[1] - p0[1], 0};
		return v;
	}
	else{
